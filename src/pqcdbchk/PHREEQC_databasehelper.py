...
class SearchPageBase(tk.Frame):
    def __init__(self, master, title, category):
        ...
        tk.Checkbutton(search_frame, text="Exact Match", variable=self.exact_var).pack(
            side="left"
        )

        btn_frame = tk.Frame(self)
        btn_frame.pack(pady=10)
        tk.Button(btn_frame, text="Search", command=self.perform_search, width=15).pack(
            side="left", padx=5
        )
        tk.Button(
            btn_frame,
        ...
        tk.Button(
            btn_frame,
            text="Back",
            command=lambda: master.show_frame(FindValuesPage),
            width=10,
        ).pack(side="left", padx=5)
        ...
        self.tree = ttk.Treeview(
            self.tree_frame, selectmode="extended", yscrollcommand=yscroll.set
        )
    ...
    def perform_search(self):
        ...
        self.result_df = self.searcher.search(
            self.category, query, self.exact_var.get()
        )
        ...
        for c in self.result_df.columns:
            self.tree.heading(
                c,
                text=c,
                command=lambda _c=c: treeview_sort_column(self.tree, _c, False),
            )
        ...
        if not selected_items:
            if hasattr(self, "result_df") and not self.result_df.empty:
                if not messagebox.askyesno(
                    "Export", "No items selected. Export all results?"
                ):
                    return
...
class FindEquationsPage(SearchPageBase):
    def __init__(self, master):
        super().__init__(master, "Find Species Equation", "equation")


class FindSpeciesPage(SearchPageBase):
    def __init__(self, master):
        super().__init__(master, "Find Solution Master Species", "species")


class FindPhasesPage(SearchPageBase):
    def __init__(self, master):
        super().__init__(master, "Find Phases", "phase")
...
class Main(tk.Frame):
    def __init__(self, master):
        super().__init__(master)
        tk.Label(self, text="PHREEQC Menu", font=("DejaVu Sans Mono", 24, "bold")).pack(
            pady=20
        )
        tk.Button(
            self,
            ...
            width=25,
            bg="#4F8FC0",
            command=lambda: master.show_frame(EditDatabasePage),
        ).pack(pady=10)
        tk.Button(
            self,
            text="Find Values",
...
class FindValuesPage(tk.Frame):
    def __init__(self, master):
        ...
        for text, page in [
            ("Find Equation", FindEquationsPage),
            ("Find Species", FindSpeciesPage),
            ("Find Phase", FindPhasesPage),
        ]:
            tk.Button(
                self, text=text, width=20, command=lambda p=page: master.show_frame(p)
            ).pack(pady=5)
...
class EditDatabasePage(tk.Frame):
    def __init__(self, master):
        ...
        self.entry_db.pack()
        tk.Button(
            inner, text="Browse", command=lambda: self.browse_file(self.entry_db)
        ).pack()
        ...
        self.entry_new.pack()
        tk.Button(
            inner, text="Browse", command=lambda: self.browse_save(self.entry_new)
        ).pack()
        ...
        tk.Button(
            inner, text="Add/Edit Species", command=self.add_species, bg="#DFF0D8"
        ).pack(pady=5)
        ...
        tk.Button(
            inner, text="Add/Edit Phase", command=self.add_phase, bg="#DFF0D8"
        ).pack(pady=5)

        tk.Button(
            inner, text="Back", command=lambda: master.show_frame(Main), bg="#F2DEDE"
        ).pack(pady=10)
    ...
    def add_species(self):
        ...
        if index != -1:
            choice = messagebox.askyesno(
                "Duplicate found", f"Species '{eq}' already exists.\nChange?"
            )
            if choice:
                end = index + 1
    ...
    def add_master_species(self):
        ...
        if index != -1:
            ...
            if choice:
                ...
                messagebox.showinfo(
                    "Edited", f"{element} master species is now edited."
                )
            else:
                return
        else:
    ...
    def add_phase(self):
        ...
        if index != -1:
            choice = messagebox.askyesno(
                "Duplicate found", f"Phase '{name}' already exists.\nChange?"
            )
            if choice:
                end = index + 1
                while end < len(lines) and lines[end].strip():
