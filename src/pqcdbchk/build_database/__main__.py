...
def main(verbose=False):
    ...
    result_sp = soln_species[
        soln_species["source"] == utils.remove_hash_from_source(db_list[0])
    ]
    ...
    for i in range(1, len(db_list)):
        ...
        equations = soln_species[
            soln_species["source"] == utils.remove_hash_from_source(db_list[i])
        ]["equation"]
    ...
    drop_re = re.compile(
        "Hg[(]OH[)]2|Sb[(]OH[)]6-|H4[(]SiO4[)]|H2[(]PO4[)]-|H3BO3|H4SiO4"
    )
    ...
    if verbose:
        logging.info(
            f"Master database compiled with {len(result_mst)} elements and {len(result_sp)} equations."
        )
    ...
    parser = argparse.ArgumentParser(
        description="Compile multiple databases into a single master database."
    )
    parser.add_argument(
        "--output", "-o", type=str, default=default_output, help="Output file path"
    )
